using ApiProjeKampi.WebUl.Dtos.AboutDtos;
...
namespace ApiProjeKampi.WebUl.Controllers
{
    public class AboutController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7003/api/Abouts?id=" + id);
            return RedirectToAction("AboutList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7003/api/Abouts/GetAbout?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateAbout(UpdateAboutDto updateAboutDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7003/api/Abouts/", stringContent);
            return RedirectToAction("AboutList");
        }
    }
}
